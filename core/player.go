...
import (
	"fmt"
	"github.com/cold-runner/mmo-game/pb/gen"
...
func (p *Player) SyncSurrounding() {
	...
	playersData := make([]*gen.Player, 0, len(players))
	for _, player := range players {
		p := &gen.Player{
			Pid: player.Pid,
			P: &gen.Position{
	...
	SyncPlayersMsg := &gen.SyncPlayers{
		Ps: playersData[:],
	}
...
// 刷新AOI视野
func (p *Player) refreshAOI(x, y, z, v float32) {
	// 1. 我离开旧的九宫格其他玩家的视野
	// 2. 旧的九宫格其他玩家消失在我的视野中
	// 3. 我出现在新的九宫格中的玩家视野中
	// 4. 新的九宫格的玩家出现在我的视野中
	...
	// 获取旧九宫格所有玩家
	oldPlayerIdList := WorldMgrObj.AoiMgr.GetPidsByPos(p.X, p.Z)
